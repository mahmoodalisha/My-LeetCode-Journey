//(i,j) glass me -> (i-1,j-1) up left se champage aaya + (i-1,j) up right se champagne aaya
class Solution {
public:
    double t[101][101];
    double solve(int poured, int i, int j){ //ith row jth column aur j kabhi i se zyada nahi ho sakta 
        if(i<0 || j<0 || i<j){
            return 0.0;
        }
